...
NAPI_API_DEFINE(NodeRtcEngine, enableVirtualBackground) {
  ...
  uint32 background_source_type, blur_degree, modeltype, modeltypeSegpropert;
  ...
  do {
    ...
    status = napi_get_object_property_uint32_(isolate, backgroundSourceObj, "background_source_type", background_source_type);
    backgroundSource.background_source_type = (VirtualBackgroundSource::BACKGROUND_SOURCE_TYPE)background_source_type;
    CHECK_NAPI_STATUS(pEngine, status);

    status = napi_get_object_property_uint32_(isolate, backgroundSourceObj, "color",
      backgroundSource.color);
    CHECK_NAPI_STATUS(pEngine, status);

    status = napi_get_object_property_nodestring_(isolate, backgroundSourceObj, "source", source);
    backgroundSource.source = source;
    CHECK_NAPI_STATUS(pEngine, status);

    status = napi_get_object_property_uint32_(isolate, backgroundSourceObj, "blur_degree",
      blur_degree);
    backgroundSource.blur_degree = (VirtualBackgroundSource::BACKGROUND_BLUR_DEGREE)blur_degree;
    CHECK_NAPI_STATUS(pEngine, status);

    status = napi_get_object_property_uint32_(isolate, backgroundSourceObj, "modeltype",
      modeltype);
    backgroundSource.modeltype = (VirtualBackgroundSource::SEG_MODEL_TYPE)modeltype;
    CHECK_NAPI_STATUS(pEngine, status);

    status = napi_get_object_property_int32_(isolate, backgroundSourceObj, "prefervelocity",
      backgroundSource.prefervelocity);
    CHECK_NAPI_STATUS(pEngine, status);

    status = napi_get_object_property_double_(isolate, backgroundSourceObj, "greencapacity", greencapacity);
    backgroundSource.greencapacity = greencapacity;
    CHECK_NAPI_STATUS(pEngine, status);
    ...
    status = napi_get_object_property_uint32_(isolate, segpropertObj, "modeltype",
      modeltypeSegpropert);
    segpropert.modeltype = (SegmentationProperty::SEG_MODEL_TYPE)modeltypeSegpropert;
    CHECK_NAPI_STATUS(pEngine, status);

    status = napi_get_object_property_int32_(isolate, segpropertObj, "prefervelocity",
      segpropert.prefervelocity);
    CHECK_NAPI_STATUS(pEngine, status);

    status = napi_get_object_property_double_(isolate, segpropertObj, "greencapacity", greencapacitySegpropert);
    segpropert.greencapacity = greencapacitySegpropert;
    CHECK_NAPI_STATUS(pEngine, status);

    result = pEngine->m_engine->enableVirtualBackground(enabled, backgroundSource, segpropert);
  } while (false);
  napi_set_int_result(args, result);
  LOG_LEAVE;
}
