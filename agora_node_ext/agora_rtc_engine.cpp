...
NAPI_API_DEFINE(NodeRtcEngine, videoSourceSetLocalAccessPoint) {
  ...
  napi_status status = napi_ok;
  int result = -1;
  std::unique_ptr<LocalAccessPointConfigurationCmd> cmd(new LocalAccessPointConfigurationCmd());

  std::string pattern = "videoSourceSetLocalAccessPointpattern";
  do {
    Isolate *isolate = args.GetIsolate();
    Local<Context> context = isolate->GetCurrentContext();
    NodeRtcEngine *pEngine = nullptr;
    ...
    if (status == napi_ok && ipList->Length() > 0) {
      for (uint32 i = 0; i < ipList->Length(); i++) {
        ...
        status = napi_get_value_nodestring_(value, str);
        CHECK_NAPI_STATUS(pEngine, status);
        ipListStr += string(str) + pattern;
      }
    }
    cmd->ipList = ipListStr.c_str();
    ...
    if (status == napi_ok && domainList->Length() > 0) {
      for (uint32 i = 0; i < domainList->Length(); i++) {
        ...
        status = napi_get_value_nodestring_(value, str);
        CHECK_NAPI_STATUS(pEngine, status);
        domainListStr += string(str) + pattern;
      }
    }
    ...
    if (pEngine->m_videoSourceSink.get()) {
      pEngine->m_videoSourceSink->setLocalAccessPoint(localAccessPointConfiguration);
      result = 0;
    }
  } while (false);
  napi_set_int_result(args, result);
